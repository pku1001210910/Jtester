...
public class LivVarCalculator extends Calculator implements IJob {
	public void initAnalysisBlocks(TestData data) {
		...
		while (itr.hasNext()) {
			...
			AnalysisBlock alsBlock = new LivVarBlock(bb, i++);
	...
	public void calculateSolution() {
		while (this.workList.size() != 0) {
			...
			if (!isVisited(second)) {
				...
				// 求并集
				ArrayList<Object> exit = computeUnion(this.getEntry(first), this.getExit(second));
	...
	protected void transfer(int lab) {
		for (int i = 0; i < this.analysisBlocks.size(); i++) {
			if (this.analysisBlocks.get(i).getLable() == lab) {
				((LivVarBlock) this.analysisBlocks.get(i)).tansfer();
				break;
			}
		}
	}
